...
static void fillData(std::string *arr, bool *active){
  int index = 0;
  std::queue<Node*> queue{};
  queue.push(Scene::getRoot());
  for(Node* n=queue.front(); !queue.empty(); n=queue.front()){
      ...
      arr[index++] = createIndent(rank) + "#11" + std::to_string(6 - n->collapsed) + "#" +  n->name;
      if(n->collapsed)
          continue;
      auto& childs = n->getChildren();
      for(auto& child: childs)
          queue.push(child.get());
      auto& guis = n->getGuis();
      std::string indent = createIndent(rank+3);
      for(auto& gui: guis){
        if(Tool::selectedGui)
          active[index] = (Tool::selectGUI.find(gui.get()) != Tool::selectGUI.end());
        arr[index++] = indent +  gui->getLabel();
      }
  }
}
...
static void SceneTree(){
  ...
  BeginScissorMode(sRec.x, sRec.y, sRec.width, sRec.height);
  {
    ...
    for (int i = 0; i < itemCnt; i++){
      ...
      raygui::GuiToggle(treeView[i], labels[i].c_str(),&active);
      if(active ^ actives[i])
        choosen = i;
    }
